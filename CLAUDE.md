...
- `pages/Chats.jsx` — layout: header (52px) + `chat-shell` flex-row → `chat-main` (messages + input) + right collapsible sidebar (240px/40px, toggle chevron); sidebar: vertical chat list + Clear Chat + Delete All History buttons; SpyderPanel: Recharts bar/line/pie + KPI tiles + data tables; export menu: CSV/Excel/PDF snapshot/Branded Report; 👍👎 feedback; **Copy response** button (copies text or synthesis+SQL as TSV, 2s "Copied" state); user bubble: subtle bg (no blue); `prompt` prop from prior user message
- `pages/UserProfile.jsx` — two-column flex-wrap layout: profile card (read-only name/email/role/SGs) left + change password form right; wraps on narrow screens
- `pages/AgentManagement.jsx` — 3-section layout: **Agents** (two-panel config editor); **Observability** (KPI tiles with sample data: 1,247 req · 4.2s latency · 3.1% error · 8.4% guardrail blocks · 2.3 avg intents; agent health cards; latency/volume chart placeholders; 8-row pipeline traces table with `TraceStatusBadge` — ✓ Pass / ⟳ Corrected / 🚫 Blocked / ✗ Failed / ⚠ Error); **Evaluation** (feedback KPIs 89👍/12👎/88.1% satisfaction + 5-row log; SQL quality 74.2% first-pass/25.8% correction/91.3% success + 5-row violations table with `ViolationTypeBadge` CLS/RLS/SCHEMA/SYNTAX/FILTER; intent classification 2.3 avg/58.2% structured/21.4% unstructured/20.4% both; RAG quality 0.847 similarity/4.2% zero-result/3.8 avg chunks — all sample data, backend wiring parked)
- `pages/AppMapping.jsx` — Application User Mapping (admin only, dummy/frontend-only): **top dropdown selector bar** (app dropdown + auth badge + active toggle + base URL + Add API/Delete); detail: user selector, per-API Read/Write checkboxes, auth-type credential fields (Basic/API Key/OAuth2) with show/hide; Add Application + Add API modals; no backend wiring yet
- `pages/AgentSkills.jsx` — Custom Agent Skills (admin only, dummy/frontend-only): **top selector bar** (search + All/View/Template filter pills + skill dropdown + New Skill button); editor: name/type/domain, description, SQL textarea, parameters table, invocation preview, save/discard
